...
import static io.github.archessmn.ENG1.Interface.GameScreen.MAP_WIDTH;

/**
 * Utilities to assist with usage of the world grid.
 */
public class GridUtils {
    public final static int GRID_WIDTH = 11;
    ...
    public static Vector2 getGridSquareScreenCoords(GridCoordTuple coords) {
        float viewportX = (float)MAP_WIDTH * coords.x / GRID_WIDTH;
        float viewportY = (float)VIEWPORT_HEIGHT * coords.y / GRID_HEIGHT;
        return new Vector2(viewportX, viewportY);
    }

    /**
     * Get the grid square containing the given coordinates. The top left grid position is (0, GRID_HEIGHT - 1).
    ...
    int gridX = (int)((x / MAP_WIDTH) * GRID_WIDTH);
